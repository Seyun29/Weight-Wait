...
/* const myReserve = () => { //route추가 8/7
  //석우꺼
  fetch("https://so6wenvyg8.execute-api.ap-northeast-2.amazonaws.com/dev/reservation?userid=116539218716908666669", {
           method: 'GET',
           headers: {
...
const ReserveScreen = ({navigation,route, category}) => { //navigation,route 추가 8/6
  ...
  const myReserve = () => { //route추가 8/7  8/8에 지움
    //석우꺼
    const getuserid = async() => {
    try {
      const value = await AsyncStorage.getItem('@storage_userid');
      if(value!==null){
        const url="https://so6wenvyg8.execute-api.ap-northeast-2.amazonaws.com/dev/reservation?userid="+value
        fetch(url, {
          method: 'GET',
          headers: {
          Accept: 'application/json',
