...
const ReserveScreen = ({navigation, route, category}) => {
  ...
  useEffect( () => {
    ...
    .then((json) => {
      ...
      for(var h in machinelist){
        console.log(machinelist[h]);
      }
      console.log("return?");
     ...
     /* json.machinearr.forEach(element => {
          let id1=-1;
          let name1='';
          let category1=-1;
          let waitnum1=-1;
          for(let i=0;i<element.length-2;i++){
            switch(i){
              case 0: id1=Number(element[i]);break;//machineid
              case 1: name1=element[i];break;//machinename
              case 2: category1=Number(element[i]);break;//machinecategory
              case 3: waitnum1=2;break;//machineimage
            }
          let object={name:name1,id:id1,waitnum:waitnum1,category:category1};
            //switch case로 i=1이면 dictobject[name]=element[i]이런식으로 구성
          machinelist.push(object);
          } */
    .catch((error) => {
      console.error(error);
...
console.log(machine);
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: 'white'}}>
      ...
      <MachineView machine={machine} handlerFunction={handlechange} change1={change1}></MachineView>
